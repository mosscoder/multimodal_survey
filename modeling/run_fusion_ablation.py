"""Does a LEARNED fusion recover any residual drone value the fixed additive sum
was losing? Same ground top-1 branch + aerial linear branch throughout; only the
combination rule changes. Trained together (5 modes x 3 seeds = 15 heads/fold,
shared batch stream), same data/folds/optimizer as the modality ablation.

  ground_only   -- reference, no aerial term
  sum_fixed     -- the original naive additive sum (= the earlier "concat")
  gate_scalar   -- ground + gate*aerial, ONE learnable scalar, init 0
  gate_perclass -- ground + gate*aerial, PER-CLASS learnable gate (8 params), init 0
  fusion_mlp    -- concat([ground_scores, aerial_scores]) -> small MLP -> logits
"""
import json
import numpy as np
import pandas as pd
...
MODES = ["ground_only", "sum_fixed", "gate_scalar", "gate_perclass", "fusion_mlp"]
SEEDS = [0, 1, 2]
P_PATCHES = 3072
N_OUT = 8
...
class FusionHead(nn.Module):
    def __init__(self, mode, d_g=768, d_a=1024, n_out=N_OUT):
        super().__init__()
        self.mode = mode
        self.ground = nn.Linear(d_g, n_out, bias=(mode == "ground_only" or mode.startswith("gate") or mode == "fusion_mlp"))
        if mode != "ground_only":
            self.aerial = nn.Linear(d_a, n_out)
        if mode == "gate_scalar":
            self.gate = nn.Parameter(torch.zeros(1))
        elif mode == "gate_perclass":
            self.gate = nn.Parameter(torch.zeros(n_out))
        elif mode == "fusion_mlp":
            self.fuse = nn.Sequential(nn.Linear(2 * n_out, 16), nn.GELU(), nn.Linear(16, n_out))

    def forward(self, G, a):
        gs = self.ground(G).topk(1, dim=1).values.mean(1)
        if self.mode == "ground_only":
            return gs
        as_ = self.aerial(a)
        if self.mode == "sum_fixed":
            return gs + as_
        if self.mode in ("gate_scalar", "gate_perclass"):
            return gs + self.gate * as_
        return self.fuse(torch.cat([gs, as_], dim=1))  # fusion_mlp
...
def fit_predict_mil(rows_tr, rows_va, lr=1e-2, wd=1e-2, epochs=60, patience=10, batch=64):
    ...
    for h in [(mode, s) for mode in MODES for s in SEEDS]:
        torch.manual_seed(h[1]); heads[h] = FusionHead(h[0]).cuda()
    ...
    for h, m in heads.items():
        ...
        out[h] = predict(m)
        if h[0] in ("gate_scalar", "gate_perclass"):
            gates[h] = m.gate.detach().cpu().numpy().copy()
...
for f in range(K):
    ...
    preds, gates = fit_predict_mil(abs_idx[tr], abs_idx[va])
    for h, Pv in preds.items():
        oof[h][va] = Pv
    for h, g in gates.items():
        learned_gates.append(dict(fold=f, mode=h[0], seed=h[1], gate=list(g)))

np.savez("results/fusion_ablation_oof.npz", **{f"{m}_s{s}": oof[(m, s)] for m in MODES for s in SEEDS})
pd.DataFrame(learned_gates).to_csv("results/fusion_ablation_gates.csv", index=False)
...
for mode in MODES:
    ...
    for label, P in list(zip([f"{mode} s{s}" for s in SEEDS], seed_P)) + [(f"{mode} ENS", np.mean(seed_P, 0))]:
        ...
        print(f"{label:18s} macroF1 {per_f1.mean():.3f}  macroAP {per_ap.mean():.3f}  "
...
print("\n=== learned gate values (gate_scalar / gate_perclass) ===")
print(pd.DataFrame(learned_gates).to_string(index=False))
df.to_csv("results/fusion_ablation.csv", index=False)
pd.DataFrame(apr).to_csv("results/fusion_ablation_perclass_ap.csv", index=False)
print("\nwrote results/fusion_ablation.csv")
